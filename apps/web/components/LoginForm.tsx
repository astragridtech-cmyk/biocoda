...
export function LoginForm({ tenants }: { tenants: { id: string; name: string }[] }) {
  ...
  const [mode, setMode] = useState<"supabase" | "demo">(SUPABASE_ENABLED ? "supabase" : "demo");
    ...
    <div className="card w-full max-w-md p-7">
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-xs text-muted">Sign in to your portfolio</div>
        </div>
      </div>
      ...
      {mode === "supabase" && (
        <div className="mb-4 space-y-2">
          <button
            ...
            disabled={busy}
            className="flex w-full items-center justify-center gap-2.5 rounded-md border border-line bg-white px-4 py-2 text-sm font-medium text-ink hover:bg-panel disabled:opacity-50"
          >
        ...
        <form onSubmit={supabaseSignIn} className="space-y-3">
          <label className="block text-xs font-medium text-ink">
            ...
            <input
              ...
              placeholder="you@responsiblebody.example"
            />
          </label>
          <label className="block text-xs font-medium text-ink">
            Password
          ...
          <button
            type="submit"
            disabled={busy}
            className="w-full rounded-md bg-moss px-4 py-2 text-sm font-medium text-white hover:bg-leaf disabled:opacity-50"
          >
        ...
        <form onSubmit={demoSignIn} className="space-y-3">
          {!SUPABASE_ENABLED && (
            <p className="rounded-md bg-panel px-3 py-2 text-xs text-muted">
              Supabase Auth is not configured in this environment, so this is a demo sign-in. Set
              the Supabase env vars to enable real accounts.
            </p>
          )}
          <label className="block text-xs font-medium text-ink">
            Email (optional)
            <input
              ...
              onChange={(e) => setEmail(e.target.value)}
              className="mt-1 w-full rounded-md border border-line px-3 py-2 text-sm"
              placeholder="demo@biocoda.earth"
            />
          </label>
          <label className="block text-xs font-medium text-ink">
            Role
      ...
      {SUPABASE_ENABLED && (
        <button
          onClick={() => setMode((m) => (m === "supabase" ? "demo" : "supabase"))}
          className="mt-4 w-full text-center text-xs text-muted underline"
        >
          {mode === "supabase" ? "Continue in demo mode instead" : "Use email and password"}
        </button>
      )}
    </div>
  );
